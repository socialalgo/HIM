...
def ISpread_K_calculated():
    ...
    for K in range(1, Kmax + 1):
        ...
        rec_list = np.unique(rec_list)  # 去重
        second_coverage = []
        second_coverage.extend(rec_list)    # 一阶传播加进去
        for rec in rec_list:
...
def evaluate(Recommendation):
    ...
    edge_list = np.array(pd.read_csv("datasets/edge_list.csv", header=None))
    ...
    for src, dst, v in tqdm(test_data):
        ...
        if src in test_node_dict:
            test_user_dict[src].append([dst, v])    # 取出给test_node的推荐结果
    ...
    Spread_K_valid = dict()   # 记录推荐出来的前K个人的有效传播轨迹（一阶传播）
    for user in test_user_dict:
        Spread_K_valid[user] = [[], [], []]

    # 记录每个用户的recall@K(1, 2, 3)
    ...
    for uid, user in enumerate(test_user_dict):
        ...
        for _k in range(1, K + 1):
            ...
            algo_k = np.array(algo_rec)[:_k, 0]  # 按序取出前_k个的uid
            n_union = len(np.intersect1d(algo_k, spread_dict[user]))    # 计算交际
            ...
            Recall_K_deg[user].append(n_union / spread_num[user])   # 记录每个用户的recall@K
    ...
    Spread_K_full_user = defaultdict(list) # 记录一下每个人的s@1, s@2, s@3, 再进一步筛一下测试集
    ...
    for user in Spread_K_valid:
        for _k in range(K):
            ...
            Spread_K_full_user[user].append(len(first_order))
    ...
    ISpread_K = np.array(pd.read_csv("datasets/ISpread@K.csv", header=None)).reshape(-1, )
